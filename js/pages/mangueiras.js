...
let mangueirasRetornoAreaCache = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Listeners para soma do retorno de teste
    const elAprovadas = document.getElementById('retorno-aprovadas');
    const elReprovadas = document.getElementById('retorno-reprovadas');
    if (elAprovadas) elAprovadas.addEventListener('input', atualizarSomaRetornoTeste);
    if (elReprovadas) elReprovadas.addEventListener('input', atualizarSomaRetornoTeste);

    // Listeners para soma do retorno da área
    const elFurtadas = document.getElementById('retorno-furtadas');
    const elTeste = document.getElementById('retorno-teste');
    const elDescarte = document.getElementById('retorno-descarte');
    if (elFurtadas) elFurtadas.addEventListener('input', atualizarSomaRetorno);
    if (elTeste) elTeste.addEventListener('input', atualizarSomaRetorno);
    if (elDescarte) elDescarte.addEventListener('input', atualizarSomaRetorno);
});
...
async function carregarMangueirasParaRetornoTeste() {
    ...
    try {
        ...
        mostrarToast('Erro ao carregar mangueiras em teste', 'erro');
    } finally {
        toggleLoading(false);
    }
}
...
function atualizarSelectRetornoTeste() {
    ...
    if (!select) return;
    select.innerHTML = '<option value="">Selecione uma mangueira...</option>';
    if (mangueirasRetornoTesteCache.length === 0) {
        const opt = document.createElement('option');
        opt.value = '';
        opt.textContent = 'Nenhuma mangueira em teste no momento';
        opt.disabled = true;
        select.appendChild(opt);
        return;
    }
...
async function carregarMangueirasParaRetornoArea() {
    toggleLoading(true);
    try {
        const { data, error } = await sb
            .from('materiais')
        ...
        if (error) throw error;
        mangueirasRetornoAreaCache = data || [];
        atualizarSelectRetornoArea();
    } catch (erro) {
        console.error('Erro ao carregar mangueiras para retorno da área:', erro);
        mostrarToast('Erro ao carregar mangueiras aplicadas', 'erro');
    } finally {
        toggleLoading(false);
    }
}
...
function atualizarSelectRetornoArea() {
    ...
    if (!select) return;
    select.innerHTML = '<option value="">Selecione uma mangueira...</option>';
    if (mangueirasRetornoAreaCache.length === 0) {
        const opt = document.createElement('option');
        opt.value = '';
        opt.textContent = 'Nenhuma mangueira aplicada na área no momento';
        opt.disabled = true;
        select.appendChild(opt);
        return;
    }
    mangueirasRetornoAreaCache.forEach(m => {
        const opt = document.createElement('option');
        opt.value = m.id;
...
function atualizarInfoRetornoArea() {
    ...
    const m = mangueirasRetornoAreaCache.find(x => x.id === materialId);
    if (!m) {
        infoDiv.classList.add('hidden');
        infoSoma.classList.add('hidden');
        return;
...
function abrirModalRetornoArea() {
    ...
    carregarMangueirasParaRetornoArea();
...
window.salvarMovimentacaoMangueira = salvarMovimentacaoMangueira;
window.atualizarInfoMovimentacao = atualizarInfoMovimentacao;
window.atualizarSaldoMovimentacao = atualizarSaldoMovimentacao;
window.filtrarMovimentacoesMangueira = filtrarMovimentacoesMangueira;
window.limparFiltroMovimentacoes = limparFiltroMovimentacoes;
window.exportarMovimentacoesMangueira = exportarMovimentacoesMangueira;
window.exportarMangueiras = exportarMangueiras;
...
window.atualizarSomaRetornoTeste = atualizarSomaRetornoTeste;
...
window.atualizarSomaRetorno = atualizarSomaRetorno;
